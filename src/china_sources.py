...
CHINA_TRANSCRIPT_SUFFIXES = ["业绩会纪要", "业绩说明会纪要", "电话会纪要", "交流纪要", "调研纪要", "投资者交流纪要"]
CHINA_EXPERT_MEMO_SUFFIXES = [
    "专家交流纪要",
    "专家电话会纪要",
    "专家会议纪要",
    "行业专家交流",
...
def _sogou_wechat_link(query: str, kind: str) -> LinkResult:
    ...
    return LinkResult(title=f"微信文章搜索：{query}", url=url, source="微信公众号搜索", kind=kind, is_direct_file=False, note="打开后可查看公众号业绩会纪要、专家交流纪要、券商整理和会议记录；正式结论需交叉验证。")
...
def _wechat_site_search_link(query: str, kind: str) -> LinkResult:
    return LinkResult(
        ...
        note="备用入口：搜索 mp.weixin.qq.com 公开文章，特别关注专家交流、产业链调研和业绩会纪要。",
    )
...
def _build_queries(company: dict[str, Any], kinds: list[str] | None = None, years: list[str] | None = None, quarters: list[str] | None = None) -> list[tuple[str, str, str]]:
    ...
    if not selected or "transcript" in selected:
        for suffix in [*CHINA_TRANSCRIPT_SUFFIXES, *CHINA_EXPERT_MEMO_SUFFIXES]:
        ...
        queries.extend(
            [
                ...
                (f"site:mp.weixin.qq.com {name} 专家交流纪要 {date_text}".strip(), "微信公众号专家纪要", "transcript"),
                (f"site:mp.weixin.qq.com {name} 产业链专家交流 {date_text}".strip(), "微信公众号专家纪要", "transcript"),
                (f"site:mp.weixin.qq.com {name} 渠道调研纪要 {date_text}".strip(), "微信公众号专家纪要", "transcript"),
                (f"site:xueqiu.com {name} 业绩会纪要 {date_text}".strip(), "雪球搜索", "transcript"),
                (f"site:xueqiu.com {name} 专家交流纪要 {date_text}".strip(), "雪球专家纪要", "transcript"),
...
def find_china_research_links(
    ...
    jobs = [(_duckduckgo_search, (query, source, kind, 4), {}) for query, source, kind in queries[:18]]
    results: list[LinkResult] = []
    ...
    if not kinds or "transcript" in kinds:
        wechat_queries = [
            ...
            f"{name} 专家交流纪要 {years_text}".strip(),
            f"{name} 行业专家交流 {years_text}".strip(),
            f"{name} 产业链专家交流 {years_text}".strip(),
            f"{name} 渠道调研纪要 {years_text}".strip(),
        ]
        results.extend(_sogou_wechat_link(query, "transcript") for query in wechat_queries)
        results.extend(_wechat_site_search_link(query, "transcript") for query in wechat_queries[:5])
    if not results:
        results.append(_general_search_link(f"{name} 专家交流纪要 微信公众号", "中文投研搜索", "transcript"))
